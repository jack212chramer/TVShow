...
<div class="container">
<a style="margin-bottom:100px;" href="createNews"><button>Add new article</button></a>         
...
<c:forEach items="${list}" var="innerList">       
      <tr>
        ...
        <td><a href="news?news_id=${innerList.getId()}">${innerList.getHeader()}</a></td>
        <td><c:out value="${innerList.getAuthorName()}"/></td>
        <td>       <c:if test = "${user_id == innerList.getAuthor_id()}">
        <form action="newsEdit"  method="post">
		<input type="text" value="${innerList.getId()}" name="news_id" style="display:none">
